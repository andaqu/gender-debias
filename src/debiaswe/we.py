...
class WordEmbedding:
    def __init__(self, fname):
        ...
        if "word2vec" in fname: model = gensim.models.Word2Vec.load(fname)
        else: model = gensim.models.FastText.load(fname)

        words = sorted([w for w in model.wv.vocab], key=lambda w: model.wv.vocab[w].index)
    ...
    def v_neighbors(self, v, topn=50, thresh=1):
        dots = self.vecs.dot(v)
        return {self.words[i] : dot for i, dot in enumerate(dots) if dot >= 1-thresh/2}

    def most_similar(self, a, x, b, topn=50, thresh=1):
        yv = self.v(x) + self.v(b) - self.v(a)
        r = self.v_neighbors(yv, topn)
        r.pop(b, None)
        r.pop(x, None)
        return dict(sorted(r.items(), key=lambda x: x[1], reverse=True)[:topn])
    ...
    def best_analogies_dist_thresh(self, v, thresh=1, topn=500, max_words=50000, eps=0.001):
